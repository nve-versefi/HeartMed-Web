...
export async function GET() {
    try {
        ...
        const result = await menuItemsCollection.aggregate([
            ...
            { $unwind: "$submenu" },
            { $match: { "submenu.name": "Medicina Estética Masculina" } },
            { $project: { _id: 0, title: 1, submenu: "$submenu" } }
        ]).toArray();

        if (result.length === 0) {
            return NextResponse.json({ error: 'Medicina Estética with Medicina Estética Masculina data not found' }, { status: 404 });
        }
